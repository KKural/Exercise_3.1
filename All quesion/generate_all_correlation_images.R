# =============================================================================
# DODONA CORRELATION EXERCISES - IMAGE GENERATION SCRIPT
# =============================================================================
# This script generates all visualization images for correlation exercises
# Each image is saved with custom dimensions to the appropriate media folder
# =============================================================================
...
generate_q3_image <- function() {
  ...
  p_q3 <- ggplot(crime_data, aes(x = Politieaanwezigheid, y = Drugsdelicten)) +
    ...
    labs(
      title = wrap_title("Correlatie: Politieaanwezigheid vs Drugsdelicten", 35),
  ...
  # Custom size for Q3.3: slightly smaller for compact display
  save_dodona_image(p_q3, 3, width = 4.5, height = 3.2)
  return(p_q3)
}

# QUESTION 3.8 - Interpret correlation
generate_q8_image <- function() {
  ...
  p_q8 <- ggplot(pos_cor_data, aes(x = Variable_X, y = Variable_Y)) +
    ...
    geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "darkred") +
    labs(
      title = wrap_title("Spreidingsdiagram: Relatie tussen Twee Variabelen", 35),
      subtitle = wrap_title("Observatiepatroon in criminologische data (N = 75 gemeenten)", 45),
...
generate_q11_image <- function() {
  ...
  p_q11 <- ggplot(data_long, aes(x = x, y = y)) +
    ...
    labs(
      title = wrap_title("Vier Studies met Identieke Statistieken maar Verschillende Patronen", 35),
      subtitle = paste0("Alle studies: r = ", corr, ", Gem. x = ", mean_x, 
                        ", Gem. y = ", mean_y, ", SD x = ", sd_x, ", SD y = ", sd_y),
      caption = wrap_title("Waarom samenvatting statistieken alleen onvoldoende zijn voor dataanalyse", 50),
      x = "Sociaaleconomische Factor",
      y = "Criminaliteitsmaat"
    ) +
    theme_minimal()
  
  # Larger size for Q3.11 due to 4-panel layout
  save_dodona_image(p_q11, 11, width = 6.0, height = 4.5)
  return(p_q11)
}

# QUESTION 3.12 - Covariance vs Correlation differences
generate_q12_image <- function() {
...
generate_q14_image <- function() {
  ...
  p_q14 <- ggplot(data_plot, aes(x = stress, y = agressie)) +
    ...
    labs(
      title = wrap_title("Monotone vs. Lineaire Relatie: Stress en Agressie", 35),
...
generate_q15_image <- function() {
  ...
  p_q15 <- ggplot(all_patterns, aes(x = x, y = y)) +
    ...
    labs(
      title = wrap_title("Correlatiepatronen: Richting en Sterkte Combinaties", 35),
...
generate_q19_image <- function() {
  ...
  p_q19 <- ggplot(all_data, aes(x = x, y = y)) +
    ...
    labs(
      title = wrap_title("Impact van Uitbijters op Correlatiecoëfficiënt", 35),
...
generate_q21_image <- function() {
  ...
  p_q21 <- ggplot(all_scenarios, aes(x = x, y = y)) +
    ...
    labs(
      title = wrap_title("Verschillende Correlatiescenario's voor Interpretatie", 35),
...
generate_q23_image <- function() {
  ...
  p_q23 <- ggplot(all_data, aes(x = x, y = y)) +
    ...
    facet_wrap(~ type, ncol = 2) +
    labs(
      title = wrap_title("Pearson Correlatie Aannames: Voldaan vs. Geschonden", 35),
  ...
  # Large size for Q3.23 (4-panel layout)
...
generate_all_images <- function() {
  cat("=============================================================================\n")
  cat("GENERATING ALL DODONA CORRELATION EXERCISE IMAGES\n")
  cat("=============================================================================\n")
  
  # Create list to store all plots (for potential return/inspection)
  plots <- list()
  
  # Generate all images with custom dimensions
  plots$q3 <- generate_q3_image()       # 4.5 x 3.2 - Compact for key measures
  plots$q8 <- generate_q8_image()       # 5.0 x 3.5 - Standard size
  plots$q11 <- generate_q11_image()     # 6.0 x 4.5 - Large for 4-panel
  plots$q12 <- generate_q12_image()     # 5.5 x 4.0 - Medium for 2-panel vertical
  plots$q14 <- generate_q14_image()     # 5.0 x 3.5 - Standard size
  plots$q15 <- generate_q15_image()     # 6.0 x 4.5 - Large for 4-panel
  plots$q18 <- generate_q18_image()     # 6.5 x 5.0 - Extra large for complex 4-panel
  plots$q19 <- generate_q19_image()     # 5.5 x 3.8 - Medium for 2-panel
  plots$q21 <- generate_q21_image()     # 6.0 x 4.5 - Large for 4-panel
  plots$q23 <- generate_q23_image()     # 6.0 x 4.5 - Large for 4-panel
  ...
  cat("Check the console output above for specific file paths and sizes\n")
  cat("=============================================================================\n")
