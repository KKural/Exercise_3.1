...
```{r correlatie-key-statistics, echo=FALSE, message=FALSE, warning=FALSE}
...
cat("=== BESCHRIJVENDE STATISTIEKEN ===\n")
cat("\nPolitieaanwezigheid (agenten per 1000 inwoners):")
cat(paste("\n  Gemiddelde (M):", round(mean(crime_data$Politieaanwezigheid), 2)))
cat(paste("\n  Standaarddeviatie (SD):", round(sd(crime_data$Politieaanwezigheid), 2)))
cat(paste("\n  Variantie:", round(var(crime_data$Politieaanwezigheid), 2)))

cat("\n\nDrugsdelicten (per 1000 inwoners):")
cat(paste("\n  Gemiddelde (M):", round(mean(crime_data$Drugsdelicten), 2)))
cat(paste("\n  Standaarddeviatie (SD):", round(sd(crime_data$Drugsdelicten), 2)))
cat(paste("\n  Variantie:", round(var(crime_data$Drugsdelicten), 2)))
...
cat(paste("\nCorrelatiecoëfficiënt (r):", r_value))
cat(paste("\nBepalingscoëfficiënt (R²):", r_squared))
cat(paste("\nVerklaarde variantie:", round(r_squared * 100, 1), "%"))
cat(paste("\nSteekproefomvang (n):", n))
...
if(p_value < 0.001) {
  ...
  cat("\nSignificantie: niet significant (p > .05)")
}

cat(paste("\n\n=== INTERPRETATIE ==="))
cat(paste("\nDe correlatie van r =", r_value, "betekent een", 
         if(abs(r_value) >= 0.7) "sterke" else if(abs(r_value) >= 0.5) "matige" else "zwakke",
         if(r_value > 0) "positieve" else "negatieve", "relatie."))
cat(paste("\nR² =", r_squared, "betekent dat", round(r_squared * 100, 1), 
         "% van de variantie in drugsdelicten wordt verklaard door politieaanwezigheid."))
```
...
### Vraag Q8 (Begrijpen)
**Een criminoloog onderzoekt de relatie tussen buurt sociale cohesie (onderlinge verbondenheid en samenwerking tussen bewoners) en misdaadpreventie effectiviteit in 75 Belgische gemeenten. Na het verzamelen van data over gemeenschapsbetrokkenheid en lokale veiligheidsprogramma's, berekent ze een correlatie van r = 0,75. Hoe moet zij deze bevinding interpreteren voor haar rapport aan het Ministerie van Justitie en Veiligheid?**
...
ggplot(data_plot, aes(x = stress, y = agressie)) +
...
- **Lineair** (rechte lijn): Constante verandering - als X met 1 stijgt, stijgt Y altijd met hetzelfde bedrag
- **Monotoon** (altijd dezelfde richting): Consistent toenemend of afnemend, maar mag buigen zoals een curve
- **Pearson correlatie**: Meet alleen lineaire (rechte lijn) associaties
- **Spearman correlatie**: Meet monotone (altijd dezelfde richting) associaties, ook als ze buigen

> **Hint:** Onderscheid "lineair" (rechte lijn) van "monotoon" (altijd dezelfde richting). Welke techniek kan gebogen maar consistente patronen vastleggen?
...
2) r = 0,56  
"2" = " ❌ Fout. Dit zou een matige relatie worden beschouwd, sterker dan 'zwak'."

3) r = −0,72  
"3" = " ❌ Fout. Dat is een sterke negatieve relatie, niet zwak positief, wat suggereert dat als de ene variabele stijgt, de andere scherp daalt."

4) r = 0,93  
"4" = " ❌ Fout. Dit is een extreem sterke positieve relatie, bijna perfect — veel sterker dan 'zwak'."
...
```{r correlatie-types, echo=FALSE, fig.width=12, fig.height=8}
# Genereer voorbeelddata voor verschillende correlatie types
set.seed(123)
n <- 30

# Dataset 1: Perfecte lineaire relatie (geschikt voor Pearson)
x1 <- seq(1, 10, length.out = n)
y1 <- 2 * x1 + rnorm(n, 0, 0.5)

# Dataset 2: Monotone maar niet-lineaire relatie (beter voor Spearman)
x2 <- seq(1, 10, length.out = n)
y2 <- log(x2) * 3 + rnorm(n, 0, 0.3)

# Dataset 3: Ordinale data met veel gelijke waarden (geschikt voor Kendall)
x3 <- round(runif(n, 1, 5))
y3 <- round(x3 + rnorm(n, 0, 1))

# Bereken correlaties
pearson_1 <- round(cor(x1, y1, method = "pearson"), 3)
spearman_1 <- round(cor(x1, y1, method = "spearman"), 3)
kendall_1 <- round(cor(x1, y1, method = "kendall"), 3)

pearson_2 <- round(cor(x2, y2, method = "pearson"), 3)
spearman_2 <- round(cor(x2, y2, method = "spearman"), 3)
kendall_2 <- round(cor(x2, y2, method = "kendall"), 3)

pearson_3 <- round(cor(x3, y3, method = "pearson"), 3)
spearman_3 <- round(cor(x3, y3, method = "spearman"), 3)
kendall_3 <- round(cor(x3, y3, method = "kendall"), 3)

# Maak data frames
data1 <- data.frame(x = x1, y = y1, type = "Lineaire Data")
data2 <- data.frame(x = x2, y = y2, type = "Monotone (Niet-lineaire) Data") 
data3 <- data.frame(x = x3, y = y3, type = "Ordinale Data met Gelijke Waarden")

# Combineer alle data
all_data <- rbind(data1, data2, data3)

# Maak plots
p1 <- ggplot(all_data, aes(x = x, y = y)) +
  geom_point(size = 3, color = "steelblue", alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, color = "red", formula = y ~ x) +
  facet_wrap(~ type, scales = "free", ncol = 3) +
  labs(
    title = "Verschillende Datatypes en Hun Correlaties",
    subtitle = "Rood lijn toont lineaire trend",
    x = "X variabele",
    y = "Y variabele"
  ) +
  theme_minimal() +
...
# Maak vergelijkingstabel
correlatie_tabel <- data.frame(
  `Data Type` = c("Lineaire Data", "Monotone Data", "Ordinale Data"),
  `Pearson r` = c(pearson_1, pearson_2, pearson_3),
  `Spearman ρ` = c(spearman_1, spearman_2, spearman_3),
  `Kendall τ` = c(kendall_1, kendall_2, kendall_3),
  check.names = FALSE
)
...
# Maak informatietabel over correlatie types
library(kableExtra)
info_tabel <- data.frame(
  `Correlatie Type` = c("Pearson r", "Spearman ρ (rho)", "Kendall τ (tau)"),
  `Data Type` = c("Interval/Ratio", "Ordinaal/Ranked", "Ordinaal/Kleine steekproef"),
  `Veronderstelling` = c("Lineaire relatie", "Monotone relatie", "Monotone relatie"),
  `Berekening` = c("Werkelijke waarden", "Rangnummers", "Concordante paren"),
  `Voordeel` = c("Meest krachtig", "Robuust voor uitschieters", "Goed voor kleine N"),
  check.names = FALSE
)

kable(info_tabel, caption = "Vergelijking van Correlatie Types") %>%
...
# Maak beslissingstabel
library(kableExtra)
beslissing_tabel <- data.frame(
  `Situatie` = c("Lineaire relatie, continue data", 
                 "Niet-lineaire maar monotone relatie", 
                 "Data met extreme uitschieters",
                 "Ordinale data (bijv. Likert schalen)"),
  `Beste Keuze` = c("Pearson r", "Spearman ρ", "Spearman ρ", "Spearman ρ"),
  `Reden` = c("Maximaliseert kracht voor lineaire relaties",
              "Vangt monotone trends beter op dan lineaire fit",
              "Robuust - gebruikt rangnummers, niet extreme waarden",
              "Geschikt voor ordinale/gerangschikte data"),
  check.names = FALSE
)

kable(beslissing_tabel, caption = "Wanneer Welke Correlatie Gebruiken?") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
ggplot(uitschieter_data, aes(x = x, y = y, color = uitschieter)) +
...
```{r waarom-standardisatie, echo=FALSE, fig.width=12, fig.height=8}
# Genereer voorbeelddata
set.seed(42)
n <- 50

# Oorspronkelijke data met verschillende schalen
inkomen <- runif(n, 20000, 80000)  # Euro's (20k-80k)
misdrijven <- 100 - inkomen/1000 + rnorm(n, 0, 5)  # Aantal misdrijven per 1000 inwoners
misdrijven <- pmax(0, misdrijven)  # Geen negatieve misdrijven

# Bereken z-scores (gestandaardiseerde waarden)
z_inkomen <- scale(inkomen)[,1]
z_misdrijven <- scale(misdrijven)[,1]

# Bereken correlaties
cor_origineel <- round(cor(inkomen, misdrijven), 3)
cor_z_scores <- round(cor(z_inkomen, z_misdrijven), 3)

# Maak data frame voor plotting
data_origineel <- data.frame(
  x = inkomen,
  y = misdrijven,
  type = "Oorspronkelijke Data",
  x_label = "Inkomen (€)",
  y_label = "Misdrijven per 1000 inwoners"
)

data_z_scores <- data.frame(
  x = z_inkomen,
  y = z_misdrijven,
  type = "Gestandaardiseerde Data (z-scores)",
  x_label = "Z-score Inkomen",
  y_label = "Z-score Misdrijven"
)

# Combineer data
all_data <- rbind(data_origineel, data_z_scores)
...
p1 <- ggplot(all_data, aes(x = x, y = y)) +
  ...
  facet_wrap(~ type, scales = "free", ncol = 2) +
  labs(
    title = "Oorspronkelijke Data vs Gestandaardiseerde Data",
    subtitle = paste0("Beide hebben identieke correlatie: r = ", cor_origineel),
    x = "X variabele",
    y = "Y variabele",
    caption = "Standardisatie verandert de correlatie niet, maar maakt vergelijking mogelijk"
  ) +
  theme_minimal() +
...
# Toon de wiskundige formules
library(kableExtra)
formule_uitleg <- data.frame(
  `Stap` = c("1. Bereken gemiddelden", "2. Bereken standaardafwijkingen", 
             "3. Standardiseer beide variabelen", "4. Bereken correlatie"),
  `Formule` = c("X̄ = Σx/n, Ȳ = Σy/n", 
                "sx = √[Σ(x-X̄)²/(n-1)], sy = √[Σ(y-Ȳ)²/(n-1)]",
                "zx = (x-X̄)/sx, zy = (y-Ȳ)/sy", 
                "r = Σ(zx × zy)/(n-1)"),
  `Waarom` = c("Centrale punt bepalen", 
               "Spreiding meten", 
               "Omzetten naar standaardschaal", 
               "Meet lineaire samenhang"),
  check.names = FALSE
)

kable(formule_uitleg, caption = "Waarom Correlatie Standardisatie Gebruikt") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

# Demonstreer schaalprobleem
schaal_voorbeeld <- data.frame(
  `Scenario` = c("Zonder Standardisatie", "Met Standardisatie"),
  `Inkomen Bereik` = c("€20,000 - €80,000", "z-scores: -1.5 tot +1.5"),
  `Misdrijven Bereik` = c("10 - 80 per 1000", "z-scores: -1.5 tot +1.5"),
  `Probleem` = c("Verschillende eenheden en schalen", "Beide op dezelfde schaal"),
  `Correlatie Formule` = c("Onmogelijk direct te vergelijken", "r = Σ(zx × zy)/(n-1)"),
  check.names = FALSE
)

kable(schaal_voorbeeld, caption = "Waarom Verschillende Schalen Problematisch Zijn") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
crime_rate <- -data_matrix[,2] * 3 + 15  # Crime rate, make it positive
...
p1 <- ggplot(plot_data, aes(x = SES, y = Crime)) +
  geom_point(size = 3, color = "steelblue", alpha = 0.7) +
  ...
  labs(
    ...
    caption = "Een moderaat negatieve correlatie"
  ) +
  theme_minimal() +
...
# Maak interpretatie overzicht
library(kableExtra)
interpretatie_gids <- data.frame(
  `Correlatie Bereik` = c("0.00 - 0.19", "0.20 - 0.39", "0.40 - 0.59", "0.60 - 0.79", "0.80 - 1.00"),
  `Interpretatie` = c("Zeer zwak", "Zwak", "Matig", "Sterk", "Zeer sterk"),
  `Praktische Betekenis` = c("Verwaarloosbaar", "Klein effect", "Medium effect", "Groot effect", "Zeer groot effect"),
  `R² (% verklaard)` = c("0-4%", "4-15%", "16-35%", "36-62%", "64-100%"),
  check.names = FALSE
)

kable(interpretatie_gids, caption = "Interpretatie van Correlatie Sterkte") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

# Praktijkvoorbeeld tabel
praktijk_voorbeelden <- data.frame(
  `r waarde` = c("r = -0.40", "r = -0.40", "r = -0.40"),
  `Interpretatie Aspect` = c("Richting", "Sterkte", "Verklaarde Variantie"),
  `Betekenis` = c("Negatieve relatie: hogere SES → lagere criminaliteit",
                  "Matige sterkte: redelijk voorspelbare relatie",
                  "16% van variantie verklaard, 84% door andere factoren"),
  `Praktische Implicatie` = c("SES en criminaliteit bewegen in tegengestelde richtingen",
                             "SES is een nuttige maar niet perfect voorspeller", 
                             "Vele andere factoren beïnvloeden criminaliteit"),
  check.names = FALSE
)

kable(praktijk_voorbeelden, caption = "Praktische Interpretatie van r = -0.40") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# Maak overzichtstabel van veronderstellingen
library(kableExtra)
aannames_tabel <- data.frame(
  `Veronderstelling` = c("1. Lineaire Relatie", 
                         "2. Continue Variabelen", 
                         "3. Normale Verdeling",
                         "4. Geen Extreme Uitschieters",
                         "5. Homoscedasticiteit"),
  `Betekenis` = c("De relatie tussen variabelen moet rechtlijnig zijn",
                  "Beide variabelen moeten interval of ratio meetnivaeu hebben",
                  "Data benadert normale verdeling (klokromige curve)",
                  "Geen extreme waarden die de correlatie vertekenen",
                  "Variantie rond de lijn blijft constant voor alle X-waarden"),
  `Wat Controleren` = c("Spreidingsdiagram - punten volgen rechte lijn",
                        "Meetschaal - zijn het getallen met betekenisvolle afstanden?",
                        "Histogrammen/Q-Q plots van beide variabelen",
                        "Spreidingsdiagram - zoek naar punten ver van de lijn",
                        "Residual plots - variantie moet constant blijven"),
  `Bij Schending Gebruik` = c("Spearman correlatie of data transformatie",
                             "Spearman correlatie voor ordinale data",
                             "Spearman correlatie (robuuster)",
                             "Verwijder uitschieters of gebruik Spearman",
                             "Data transformatie of robuuste methoden"),
  check.names = FALSE
)

kable(aannames_tabel, caption = "Overzicht van Pearson Correlatie Veronderstellingen") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
```{r hypothese-visualisatie, echo=FALSE, fig.width=10, fig.height=6}
# Maak gesimuleerde data voor visualisatie
set.seed(123)
n_steden <- 50

# Genereer inkomensongelijkheidsindex (Gini coëfficiënt bereik: 0.3-0.6)
gini <- runif(n_steden, 0.3, 0.6)

# Genereer geweldsmisdaadcijfers met matige-tot-sterke correlatie met Gini
geweldsmisdaad <- 2 + 25*gini + rnorm(n_steden, 0, 3)

# Maak stadslabels
stad_namen <- paste("Stad", 1:n_steden)

# Maak data frame
stad_data <- data.frame(
  Stad = stad_namen,
  Inkomens_Ongelijkheid = gini,
  Geweldsmisdaad_Cijfer = geweldsmisdaad
)

# Bereken correlatie
correlatie <- round(cor(gini, geweldsmisdaad), 2)
r_kwadraat <- round(correlatie^2 * 100, 1)

# Maak plot
ggplot(stad_data, aes(x = Inkomens_Ongelijkheid, y = Geweldsmisdaad_Cijfer)) +
  geom_point(color = "steelblue", size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "darkred") +
  labs(
    title = "Veronderstelde Relatie: Economische Ongelijkheid en Geweldsmisdaad",
    subtitle = paste0("Voorbeeld van matig-sterke positieve correlatie (r = ", 
                     correlatie, ", R² = ", r_kwadraat, "%)"),
    x = "Economische Ongelijkheid (Gini Coëfficiënt)",
    y = "Geweldsmisdaadcijfer (per 100.000 inwoners)",
    caption = "Visualisatie van een toetsbare onderzoekshypothese die voorspelde richting en sterkte toont"
  ) +
  annotate("text", x = 0.32, y = max(geweldsmisdaad) - 1, 
           label = paste0("Onderzoekshypothese Voorbeeld:\n",
                          "Steden met hogere inkomensongelijkheid (Gini coëfficiënt)\n", 
                          "zullen matig sterke positieve correlaties vertonen\n",
                          "(r = 0.40-0.60) met geweldsmisdaadcijfers per hoofd van de bevolking"),
