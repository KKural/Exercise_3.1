...
# Function to save plots to Dodona directories with screen-friendly dimensions
save_dodona_image <- function(plot_obj, question_number, filename = NULL, width_px = 380, height_px = 260) {
  if (is.null(filename)) {
  ...
  if (as.character(question_number) %in% names(question_dirs)) {
    ...
    # Calculate dimensions in inches for moderate DPI (120)
    dpi <- 120
    width_inches <- width_px / dpi
    height_inches <- height_px / dpi
    
    # Save the plot with screen-friendly dimensions
    ggsave(image_path, plot_obj, 
           width = width_inches, height = height_inches, 
           dpi = dpi, bg = "white", units = "in")
    
    cat("Saved image:", image_path, paste0("(", width_px, "x", height_px, " px)"), "\n")
  }
}
...
p_q3 <- ggplot(crime_data, aes(x = Politieaanwezigheid, y = Drugsdelicten)) +
  ...
  labs(
    title = wrap_title("Correlatie: Politieaanwezigheid vs Drugsdelicten"),
