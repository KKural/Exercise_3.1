...
# Standard theme for all correlation exercise images (matches reference style)
standard_dodona_theme <- function() {
  theme_minimal() +
    theme(
...
# Function to wrap long titles to fit in smaller images
...
p_q8 <- ggplot(pos_cor_data, aes(x = Variable_X, y = Variable_Y)) +
  ...
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "darkred") +
  labs(
    title = wrap_title("Spreidingsdiagram: Relatie tussen Twee Variabelen"),
...
ggplot(data_plot, aes(x = stress, y = agressie)) +
  ...
  labs(
    title = wrap_title("Stress vs Agressief Gedrag: Monotoon maar Niet-Lineair Patroon", 35),
