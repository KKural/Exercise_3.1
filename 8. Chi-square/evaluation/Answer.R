      ...
      testEqual(
        ...
        comparator = function(generated, expected, ...) {
          ...
          is_correct <- !is.na(student_answer) && abs(student_answer - chi_square_rounded) < 0.1
            ...
            # Feedback for incorrect answers            if (is.na(student_answer)) {
              get_reporter()$add_message("❌ Fout. Je hebt geen geldige numerieke waarde ingevoerd. Voer de chi-kwadraat waarde in als een getal, bijvoorbeeld: 8.54", type = "markdown")            } else if (abs(student_answer - 5.99) < 0.1) {
              get_reporter()$add_message(
                paste0("❌ Fout. Je lijkt de kritieke waarde van de chi-kwadraat verdeling (5.99 bij α = 0.05, df = 2) te hebben gegeven in plaats van de berekende toetsstatistiek.\n\n",
                       ...
                       "```"), type = "markdown")
              )            } else if (abs(student_answer - 2) < 0.1) {
              ...
              get_reporter()$add_message("❌ Fout. De chi-kwadraat waarde kan niet negatief zijn.", type = "markdown")            } else if (abs(student_answer - 11.89) < 0.1) {
              get_reporter()$add_message(
                paste0("❌ Fout. Je antwoord 11.89 is niet correct. De juiste chi-kwadraat waarde is ", chi_square_rounded, ".\n\n",
                       "Je hebt mogelijk een berekeningsfout gemaakt. Controleer of je de verwachte frequenties correct hebt berekend met de formule E = (rijtotaal × kolomtotaal) / totaal."), type = "markdown")
              )
            } else {              get_reporter()$add_message(
