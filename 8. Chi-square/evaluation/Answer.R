        ...
        comparator = function(generated, expected, ...) {
          ...
          if (is_correct) {
            ...
            if (is.na(student_answer)) {
              get_reporter()$add_message("❌ Fout. Je hebt geen geldige numerieke waarde ingevoerd. Voer de chi-kwadraat waarde in als een getal, bijvoorbeeld: 8.54")
              ...
              get_reporter()$add_message("❌ Fout. Je hebt het aantal vrijheidsgraden (df = 2) ingevuld in plaats van de chi-kwadraat waarde.")
            } else if (student_answer < 0) {
              get_reporter()$add_message("❌ Fout. De chi-kwadraat waarde kan niet negatief zijn.")
            } else if (abs(student_answer - 11.89) < 0.1) {
              get_reporter()$add_message(
                paste0("❌ Fout. Je antwoord 11.89 is niet correct. De juiste chi-kwadraat waarde is ", chi_square_rounded, ".\n\n",
                       "Je hebt mogelijk een berekeningsfout gemaakt. Controleer of je de verwachte frequenties correct hebt berekend met de formule E = (rijtotaal × kolomtotaal) / totaal.")
              )
            } else {
              get_reporter()$add_message(
