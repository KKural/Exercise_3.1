...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, fig.show = 'asis', results = 'hide', dev = 'png', dpi = 300)
library(knitr)
library(kableExtra)
library(ggplot2)
library(dplyr)
...
# Function to save plots to Dodona directories and images folder
save_dodona_image <- function(plot_obj, question_number, filename = NULL, width = 6, height = 4) {
  if (is.null(filename)) {
    filename_base <- paste0("correlation_plot_", question_number)
  } else {
  ...
  # Save to separate images folder (for backup/reference)
  images_dir <- "images"
  dir.create(images_dir, showWarnings = FALSE)
  
  # Save PNG to images folder
  png_path <- file.path(images_dir, paste0(filename_base, ".png"))
  ggsave(
    filename = png_path,
    plot = plot_obj,
  ...
  cat("Saved PNG to images folder:", png_path, "\n")
  
  # Save SVG to Dodona directory (for exercises)
  if (as.character(question_number) %in% names(question_dirs)) {
    dir_name <- question_dirs[as.character(question_number)]
    media_dir <- file.path(dir_name, "description", "media")
    dir.create(media_dir, recursive = TRUE, showWarnings = FALSE)
    
    svg_path <- file.path(media_dir, paste0(filename_base, ".svg"))
    ggsave(
      filename = svg_path,
      plot = plot_obj,
      width = width, height = height, units = "in",
    ...
    cat("Saved SVG for Dodona:", svg_path, "\n")
  }
}
...
dataset_labels <- c(
  ...
  "Laag-Laag Uitschieter" = paste0("Laag-Lag Uitschieter (r = ", cor2, ")"),
